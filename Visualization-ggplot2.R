...
##I want name to be displayed horizontally
##USe "las" parameter
boxplot(Iris_demo,las=2)

#RenameName each variables
boxplot(Iris_demo,las=2, names = c("S_L","S_W",
                                   "P_L","P_W",
                                   "Species"))
...
boxplot(Iris_demo,las=2, names = c("S_L","S_W",
                                   "P_L","P_W",
                                   "Species"),
        col = c("red","green","blue","Black","royalblue2"))
...
##Notched Boxplot - Display the confidence interval around
##the median
boxplot(Iris_demo$Petal.Length~Iris_demo$Species,
        xlab = "Species Type",
        ylab = "Petal Length", 
...
pie(mytable,labels = c("Setosa","Versicolor","Virginica"))
...
##size parameter in Aesthetics
  ggplot(data = mtcars, 
         aes(x=factor(cyl) , y=mpg,size = hp)) + 
    geom_point()

?ggplot(size)
##Apply multiple variables in 2 D space
ggplot(data = mtcars,
       aes(x= wt,y = mpg, col = hp,size = disp)) + 
  geom_point()
...
ggplot(tips,aes(x=total_bill,y=tip,size = size)) + geom_point()
...
temp <- ggplot(data = diamonds,
               aes(x=carat,y=price,
                   col = clarity)) +
  geom_smooth()
#temp + geom_text(data = diamonds[diamonds$carat == "2",], aes(label = clarity), hjust = 0.7, vjust = 1)
